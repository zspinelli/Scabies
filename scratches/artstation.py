...
class Artstation(Scraper):
    ...
    def __init__(self):
        super().__init__("artstation", "https://www.artstation.com")
    ...
    def _process_search_mode(self):
        for url in self._args.urls:
            ...
            query_url: str = self._domain + "/api/v2/search/projects.json"
    ...
    def _process_user_mode(self):
        for name in self._args.names:
            ...
            if self.USER_PARTS.CODES.PORTFOLIO in self._args.user_parts:
                ...
                for data in self._retrieve_paginated(self._domain + f"/users/{name}/projects.json"):
            ...
            if self.USER_PARTS.CODES.LIKES in self._args.user_parts:
                ...
                for data in self._retrieve_paginated(self._domain + f"/users/{name}/likes.json"):
                    print(data)
            ...
            if self.USER_PARTS.CODES.FOLLOWING in self._args.user_parts:
                ...
                for data in self._retrieve_paginated(self._domain + f"/users/{name}/following.json"):
            ...
            if self.USER_PARTS.CODES.FOLLOWERS in self._args.user_parts:
                ...
                for data in self._retrieve_paginated(self._domain + f"/users/{name}/followers.json"):
            ...
            if self.USER_PARTS.CODES.CHALLENGES in self._args.user_parts:
                ...
                challenges_url: str = self._domain + f"/contests/_/challenges/{name}.json"
            ...
            if self.USER_PARTS.CODES.LEARNING in self._args.user_parts:
                ...
                if self._ensure_login():
                    ...
                    user_url: str = f"{self._domain}/users/{name}.json"
                    ...
                    learning_url: str = self._domain + f"/api/v2/learning/courses/instructor_courses_by_user.json"
    ...
    def _process_learning_mode(self):
        ...
        if self._ensure_login():
            for url in self._args.urls:
                ...
                if url.startswith(f"{self._domain}/learning/courses"):
                    course_response: Response = self._sess.get(url)

                # url is a result gallery.
                elif url.startswith(f"{self._domain}/learning"):
                    params: dict = {
                        "per_page": 50,
                        "sort": "latest"
                    }
                    ...
                    courses: list = self._retrieve_paginated(
                        self._domain + "/api/v2/learning/series/search.json",
                        url,
                        params,
                        True
                    )
    ...
    def _scrape_learning(self, data: dict):
        ...
        course_url: str = f"{self._domain}/learning/{hash_id}/{slug}/"
    ...
    def _retrieve_paginated(self, url: str, ref: str = "", params: dict = {}, as_json: bool = False) -> list:
        ...
        else:   headers.update({"Referer": f"{self._domain}/"})
    ...
    def _get_csrf_token(self):
        token_response: Response = self._sess.post(
            self._domain + "/api/v2/csrf_protection/token.json",
            json={},
            headers={
                "Accept": "*/*",
                "Origin": self._domain,
    ...
    def _retrieve_project(self, hash_id: str) -> dict:
        project_url: str = self._domain + "/projects/" + hash_id + ".json"
        project_response: Response = self._sess.get(project_url)
        project: dict = project_response.json()

        # structured output wanted.
        if self._args.output_structured:
            self._destination_dir = path.join(self._args.output_structured, self._name, project["user"]["username"])
